...
#to run script: python3 LINE_TE_broadclass_random_permutations.py <peaks file> <Repeats file> <chromosome sizes file> <output file>
...
#returns dictionary with following format:
#key = feature (LINE, LINES, SINES, LTRs, RC)
...
def compare_LINE():
    ...
    for key in peaks:
        if key in final_te_dict:
            ...
            LINE_TE_dict.update({chr_num:final_chr_TE_enrichment})
            peak_overlaps_te_start = 0
            peak_overlaps_te = 0
            peak_in_te = 0
            peak_overlaps_te_end = 0
...
#final file format:
#header = Chromosome number Peak.in.TE Peak.Overlaps.TE.start Peak.Overlaps.TE.end Peak.overlaps.TE LINE.TE.Total
#last line is the total count for each element class
def combine():
    LINE_counts = compare_LINE()
    ...
    LINE_output = sys.argv[4]
    LINE_final_total = 0
    with open(LINE_output, 'a') as LINE_out:
        for value in chrs:
            single_LINE = LINE_counts[value]
            LINE_total = single_LINE[0] + single_LINE[1] + single_LINE[2] + single_LINE[3]
            LINE_final = "%s\t%s\t%s\t%s\t%s\t%s\n" % (str(value),str(single_LINE[0]),str(single_LINE[1]),str(single_LINE[2]),str(single_LINE[3]),str(LINE_total))
            LINE_out.write(LINE_final)
combine()
