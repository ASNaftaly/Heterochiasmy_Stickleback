...
#to run script: python3 SINE_TE_broadclass_random_permutations.py <peaks file> <Repeats file> <chromosome sizes file> <output file>
...
#returns dictionary with following format:
#key = feature (SINE, SINES, SINES, LTRs, RC)
...
def compare_SINE():
    ...
    for key in peaks:
        if key in final_te_dict:
            ...
            SINE_TE_dict.update({chr_num:final_chr_TE_enrichment})
            peak_overlaps_te_start = 0
            peak_overlaps_te = 0
            peak_in_te = 0
            peak_overlaps_te_end = 0
...
#final file format:
#header = Chromosome number Peak.in.TE Peak.Overlaps.TE.start Peak.Overlaps.TE.end Peak.overlaps.TE SINE.TE.Total
#last line is the total count for each element class
def combine():
    SINE_counts = compare_SINE()
    ...
    SINE_output = sys.argv[4]
    SINE_final_total = 0
    with open(SINE_output, 'a') as SINE_out:
        for value in chrs:
            single_SINE = SINE_counts[value]
            SINE_total = single_SINE[0] + single_SINE[1] + single_SINE[2] + single_SINE[3]
            SINE_final = "%s\t%s\t%s\t%s\t%s\t%s\n" % (str(value),str(single_SINE[0]),str(single_SINE[1]),str(single_SINE[2]),str(single_SINE[3]),str(SINE_total))
            SINE_out.write(SINE_final)
combine()
