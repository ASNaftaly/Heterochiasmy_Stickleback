...
#to run script: python3 RC_TE_broadclass_random_permutations.py <peaks file> <Repeats file> <chromosome sizes file> <output file>
...
#returns dictionary with following format:
#key = feature (RC, RCS, RCS, LTRs, RC)
...
def compare_RC():
    ...
    for key in peaks:
        if key in final_te_dict:
            ...
            RC_TE_dict.update({chr_num:final_chr_TE_enrichment})
            peak_overlaps_te_start = 0
            peak_overlaps_te = 0
            peak_in_te = 0
            peak_overlaps_te_end = 0
...
#final file format:
#header = Chromosome number Peak.in.TE Peak.Overlaps.TE.start Peak.Overlaps.TE.end Peak.overlaps.TE RC.TE.Total
#last line is the total count for each element class
def combine():
    RC_counts = compare_RC()
    ...
    RC_output = sys.argv[4]
    RC_final_total = 0
    with open(RC_output, 'a') as RC_out:
        for value in chrs:
            single_RC = RC_counts[value]
            RC_total = single_RC[0] + single_RC[1] + single_RC[2] + single_RC[3]
            RC_final = "%s\t%s\t%s\t%s\t%s\t%s\n" % (str(value),str(single_RC[0]),str(single_RC[1]),str(single_RC[2]),str(single_RC[3]),str(RC_total))
