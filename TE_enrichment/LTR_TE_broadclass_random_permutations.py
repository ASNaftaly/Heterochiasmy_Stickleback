...
#to run script: python3 LTR_TE_broadclass_random_permutations.py <peaks file> <Repeats file> <chromosome sizes file> <output file>
...
#returns dictionary with following format:
#key = feature (LTR, LTRS, LTRS, LTRs, RC)
...
def compare_LTR():
    ...
    for key in peaks:
        if key in final_te_dict:
            ...
            LTR_TE_dict.update({chr_num:final_chr_TE_enrichment})
            peak_overlaps_te_start = 0
            peak_overlaps_te = 0
            peak_in_te = 0
            peak_overlaps_te_end = 0
...
#final file format:
#header = Chromosome number Peak.in.TE Peak.Overlaps.TE.start Peak.Overlaps.TE.end Peak.overlaps.TE LTR.TE.Total
#last line is the total count for each element class
def combine():
    LTR_counts = compare_LTR()
    ...
    LTR_output = sys.argv[4]
    LTR_final_total = 0
    with open(LTR_output, 'a') as LTR_out:
        for value in chrs:
            single_LTR = LTR_counts[value]
            LTR_total = single_LTR[0] + single_LTR[1] + single_LTR[2] + single_LTR[3]
            LTR_final = "%s\t%s\t%s\t%s\t%s\t%s\n" % (str(value),str(single_LTR[0]),str(single_LTR[1]),str(single_LTR[2]),str(single_LTR[3]),str(LTR_total))
            LTR_out.write(LTR_final)
combine()
