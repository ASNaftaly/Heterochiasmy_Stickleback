#split peaks at TSSs into 20 bins across the chromosomes
#1. read peak file with peaks at TSSs, chromosome size file, centromere position file
#2. split chromosome into bins
#3. count the number of peaks near TSS in each bin
#4. write output to file
#to run script: python3 MakeBins_peaksatTSSs.py <peaks at TSSs> <chr num> <centromere positions file> <Chr length file> <output file>
    ...
    with open(peaks_file, 'r') as peaks:
        for line in peaks:
            ...
            if chr == chr_num:
                ...
                peak_end = new_line[2]
                if peak_start in peak_starts:
                    continue
                elif peak_start not in peak_starts:
