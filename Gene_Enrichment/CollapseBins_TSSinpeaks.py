...
#to run script: python3 CollapseBins_TSSinpeaks.py <tss in peaks sorted into bins file; output of MakeBins_peaksnearTSS_atac.py> <output>
...
def write():
    final_bins = collapse_bins()
    output = sys.argv[2]
    with open(output, 'a') as out:
        header = "Bin.Number\tTotal.Peaks.in.TSS.Counts\tPercent.Peaks.in.TSS\n"
        out.write(header)
        for bin in final_bins:
            final = "%s\t%s\t%s\n" % (str(bin), str(final_bins[bin][0]), str(final_bins[bin][1]))
            out.write(final)
